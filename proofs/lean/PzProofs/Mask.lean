...
-- mask_all: OR of bits 0-10 (read write bash edit grep find ls agent ask skill web)
-- 11 tools, bits 0-10. Mask bit indices ≠ Kind enum ordinals.
@[reducible] def mask_all : BitVec 16 := 0x7FF
...
-- Thread default mask: excludes mask_bash (bit 2) and mask_agent (bit 7).
@[reducible] def mask_bash : BitVec 16 := 1 <<< 2
@[reducible] def mask_agent : BitVec 16 := 1 <<< 7
-- 0x7FF & ~(0x04 | 0x80) = 0x7FF & 0xFF7B = 0x077B
-- = bits 0,1,3,4,5,6,8,9,10 (read,write,edit,grep,find,ls,ask,skill,web)
@[reducible] def thread_default : BitVec 16 := 0x077B
