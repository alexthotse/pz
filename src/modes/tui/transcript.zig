...
pub const MdWrapIter = struct {
    ...
    pub fn next(self: *MdWrapIter) ?[]const u8 {
        ...
        while (true) {
            ...
            if (tbl.isTableLine(line)) return line;
...
fn fillBgRow(frm: *frame.Frame, rx: usize, y: usize, rw: usize, bg: frame.Color) frame.Frame.PosError!void {
    var x = rx;
    while (x < rx + rw) : (x += 1) {
        try frm.set(x, y, ' ', .{ .bg = bg });
    }
}
