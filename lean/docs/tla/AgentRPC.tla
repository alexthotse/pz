...
\*
\* Sequence numbers are omitted from the model: they are monotonic
\* wire-level counters verified in Zig (Stub.recv rejects frame.seq <= recv_seq).
\* This spec focuses on state machine ordering, routing, and termination.
...
VARIABLES
    ...
    pipe,            \* pipe[a]   : sequence of messages parent->child
    rpipe,           \* rpipe[a]  : sequence of messages child->parent
    ...
    ran              \* ran[a]    : TRUE after first run (bounds to 1 run/agent)
...
\* Child sends Out (streaming output), bounded by MaxOut.
ChildSendOut(a) ==
    /\ cState[a] = "running"
    /\ childAlive[a]
    /\ outCnt[a] < MaxOut
...
\* Child sends Done (terminal for this run).
ChildSendDone(a) ==
    /\ cState[a] = "running"
    /\ childAlive[a]
...
\* Child sends Err (terminal for this run).
ChildSendErr(a) ==
    /\ cState[a] = "running"
    /\ childAlive[a]
    /\ cState' = [cState EXCEPT ![a] = "idle"]
...
\* Terminal: all agents have finished protocol.
Terminated ==
    \A a \in Agents :
